...
int PREV_RED_IR_CMD = 0;
int PREV_BLUE_IR_CMD = 0;
...
void updateActuator(int pinNumber)
{
  ...
  if (pinNumber + 4 == 5 and PREV_RED_IR_CMD != states[pinNumber]) {
    
    PREV_RED_IR_CMD = states[pinNumber];
    
    if (states[pinNumber] == 0) step(RED, PWM_FLT);
    
    if (states[pinNumber] == 1) step(RED, PWM_FWD1);
    if (states[pinNumber] == 2) step(RED, PWM_FWD2);
    if (states[pinNumber] == 3) step(RED, PWM_FWD3);
    if (states[pinNumber] == 4) step(RED, PWM_FWD4);
    if (states[pinNumber] == 5) step(RED, PWM_FWD5);
    if (states[pinNumber] == 6) step(RED, PWM_FWD6);
    if (states[pinNumber] == 7) step(RED, PWM_FWD7);

    if (states[pinNumber] == 100) step(RED, PWM_BRK);

    if (states[pinNumber] == 101) step(RED, PWM_REV1);
    if (states[pinNumber] == 102) step(RED, PWM_REV2);
    if (states[pinNumber] == 103) step(RED, PWM_REV3);
    if (states[pinNumber] == 104) step(RED, PWM_REV4);
    if (states[pinNumber] == 105) step(RED, PWM_REV5);
    if (states[pinNumber] == 106) step(RED, PWM_REV6);
    if (states[pinNumber] == 107) step(RED, PWM_REV7);    
    
    }
  if (pinNumber + 4 == 6 and PREV_BLUE_IR_CMD != states[pinNumber]) {
    
    PREV_BLUE_IR_CMD = states[pinNumber];
    
    if (states[pinNumber] == 0) step(BLUE, PWM_FLT);
    
    if (states[pinNumber] == 1) step(BLUE, PWM_FWD1);
    if (states[pinNumber] == 2) step(BLUE, PWM_FWD2);
    if (states[pinNumber] == 3) step(BLUE, PWM_FWD3);
    if (states[pinNumber] == 4) step(BLUE, PWM_FWD4);
    if (states[pinNumber] == 5) step(BLUE, PWM_FWD5);
    if (states[pinNumber] == 6) step(BLUE, PWM_FWD6);
    if (states[pinNumber] == 7) step(BLUE, PWM_FWD7);

    if (states[pinNumber] == 100) step(BLUE, PWM_BRK);

    if (states[pinNumber] == 101) step(BLUE, PWM_REV1);
    if (states[pinNumber] == 102) step(BLUE, PWM_REV2);
    if (states[pinNumber] == 103) step(BLUE, PWM_REV3);
    if (states[pinNumber] == 104) step(BLUE, PWM_REV4);
    if (states[pinNumber] == 105) step(BLUE, PWM_REV5);
    if (states[pinNumber] == 106) step(BLUE, PWM_REV6);
    if (states[pinNumber] == 107) step(BLUE, PWM_REV7);    
    
    
    }
  if (pinNumber + 4 != IR_pin) analogWrite(pinNumber + 4, states[pinNumber]);  
  }
}
